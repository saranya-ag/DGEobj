...
## Object Definition Attribute

The last piece of information stored as an attribute is the object definition (objDef).
This attribute holds all the pre-defined data type definitions and is updated by the
`newType` function.  

The objDef value is named list of vectors:  

```{r objDef}

names(attr(exObj, "objDef"))
...
3. uniqueType: a character vector; A list of types that can only exist once in a DGEobj.       
4. allowedLevels: a character vector;  The value of Levels defines a domain for this data.
For example, RNA-Seq data could be analyzed at the whole gene level, the isoform
level or the exon level.  

The `newType` function allows the user to define new types.  uniqueType and
allowedLevels have been pre-defined for RNA-Seq and proteomics applications.
Additional unique types or allowed levels can be added by manipulating the
objDef attribute directly.  

...
Files containing counts and sample annotations associated with this project will be downloaded from the GEO resource.  
https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE120804

Gene annotation will be downloaded from Ensembl.  
...
2. Row Data:  Additional annotations for the entities represented by each row.  For the RNA-Seq case,
this dataframe holds associated gene annotation.  The rownames in the RowData must match the rownames in 
the counts matrix.
...
With these three dataframes in hand we can initialize a DGEobj.


```{r initDGEobj}

# For now let's load the prebuilt DGEobj.
# testdataPath <- file.path(.libPaths()[1], "DGEobj", "testdata")
# dgeObj <- readRDS(fname)



# Get the raw counts and sample annotatio ("design") from GEO
...
download.file(glue("{getLocation}/{countsFile}"),
              destfile = countsFile,
              mode = 'wb')
counts <- read.delim(countsFile, stringsAsFactors = FALSE, row.names = 1)
download.file(glue("{getLocation}/{designFile}"),
              destfile = designFile,
              mode = 'wb')
design <- read.csv(designFile, stringsAsFactors = FALSE)
# The sample identifier (colnames) in the counts file is contained within
# the fastq filename column (raw.file).  Here we parse the sample identifier
# and make it the rownames to meet the specification that 
...
myDgeObj <- initDGEobj(counts  = counts,
                       rowData = gene.data,
                       colData = design,
                       level = "gene",
...
testdataPath <- file.path(.libPaths()[1], "DGEobj", "testdata")
annotationFile <- file.path(testdataPath, "GSE120804_ProjectAttributes.txt")
myDgeObj <- annotateDGEobj(myDgeObj, annotations = annotationFile) 

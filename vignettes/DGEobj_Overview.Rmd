...
```{r getType}
# ***JRT: Replace with script to download and run the workflow to generate a fully populated DGEobj.
#  How will this work?  Will the script run at package install?

# Retrieve a DGEobj with a complete workflow
fullWorkflow <- readRDS(system.file("fullWorkflow_dgeobj.RDS", package = "DGEobj", mustWork = TRUE))
MyContrasts <- getType(fullWorkflow, "topTable")
names(MyContrasts)
...
MyColData <- getBaseType(fullWorkflow, "col")
names(MyColData)
...
formula <- attr(fullWorkflow$ReplicateGroupDesign, "formula")

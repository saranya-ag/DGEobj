...
test_that('reset.R: ', {
    ...
    test_t_obj <- setAttributes(item = t_obj, list("PlatformType" = NULL))
    expect_error(resetDGEobj(test_t_obj),
                 regexp = "Required attribute \"PlatformType\" is missing.",
                 fixed  = TRUE)

    # testing t_obj with unavailable data
    test_t_obj <- setAttributes(item = t_obj, list("PlatformType" = "RNA-Seq"))
    ...
    test_t_obj1 <- setAttributes(item = t_obj, list("PlatformType" = "fred"))
